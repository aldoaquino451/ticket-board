...
<template>
  ...
  <AuthenticatedLayout>
    <!-- page name -->
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 dark:text-white leading-tight">Tickets</h2>
    </template>
    ...
    <div class="py-12">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="relative overflow-x-auto sm:rounded-lg">
          <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 sm:rounded-lg overflow-hidden shadow-sm">
            <caption class="p-5 text-lg font-semibold text-left rtl:text-right text-gray-900 bg-white dark:text-white dark:bg-gray-800">
              <p>Lista dei Ticket</p>
              <p class="mt-1 text-sm font-normal text-gray-500 dark:text-gray-400">
                Questa è lo storico di tutti i ticket che sono stati chiusi o sono in fase di completamento.
              </p>
            </caption>

            <thead class="text-xs text-gray-900 dark:text-white uppercase bg-gray-200 dark:bg-gray-700">
              <tr>
            ...
            <tbody>
              <tr v-for="ticket in ticketsArray" class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  <Link 
                    :href="route('dashboard.tickets.show', ticket.code)" 
                ...
                <td class="px-6 py-4">{{ ticket.operator ? `${ticket.operator.name} ${ticket.operator.surname}` : "Nessuno" }}</td>
                <td class="px-6 py-4 text-right">
                  <Link 
                    :href="route('dashboard.tickets.edit', ticket.code)"
                    class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                    Modifica
                  </Link>
                </td>
          ...
          <nav class="flex gap-x-8 items-center flex-column flex-wrap md:flex-row justify-between py-4" aria-label="Table navigation">
            <span class="leading-8 text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">
              Risultati da
            ...
            <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
              ...
              <li>
                <button
                  ...
                  :class="currentPage === paginationArray.last_page ? 'opacity-50 cursor-not-allowed': ''"
                  :disabled="currentPage === paginationArray.last_page"
                >
                  Successivo
                </button>
              </li>
